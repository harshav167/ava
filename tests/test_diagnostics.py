...
class TestDiagnosticTools:
    ...
    async def test_voice_mode_info(self):
        ...
        with (
            patch("voice_mode.tools.diagnostics.__version__", "2.18.0"),
            patch("pathlib.Path.exists", return_value=True),
            patch("pathlib.Path.home", return_value=Path("/home/test")),
        ):
            result = await voice_mode_info()
    ...
    async def test_check_audio_dependencies_linux(self):
        ...
        with patch(
            "voice_mode.tools.dependencies.diagnose_audio_setup", return_value=[]
        ):
            import sys
            import importlib
            ...
            try:
                ...
                with (
                    ...
                    mock_run.return_value = MagicMock(
                        returncode=0, stdout="PulseAudio 15.0"
                    )

                    result = await check_audio_dependencies()
                    ...
                    assert (
                        "packages" in result["data"] or "diagnostics" in result["data"]
                    )

                    # Should check PulseAudio
                    assert (
                        "pulseaudio" in result["data"]
                        or "diagnostics" in result["data"]
                    )
            finally:
    ...
    async def test_check_audio_dependencies_windows(self):
        ...
        with (
            ...
            mock_exists.return_value = (
                True  # /proc/sys/fs/binfmt_misc/WSLInterop exists
            )
