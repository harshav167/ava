...
from .runtime_context import EnvFileLoader, build_provider_settings as _build_provider_settings
...
def _env_file_loader() -> EnvFileLoader:
...
def find_voicemode_env_files() -> list[Path]:
    ...
    Compatibility shim for callers that still import from voice_mode.config.
    New runtime code should use RuntimeContext.env_files().
    """
    return _env_file_loader().find_files()


def parse_env_file_lines(lines: Iterable[str]) -> dict[str, str]:
    """Parse KEY=VALUE lines from voicemode env files, including multiline quotes."""
    return EnvFileLoader.parse_lines(lines)


def apply_env_file(path: Path, environ: MutableMapping[str, str] | None = None) -> dict[str, str]:
    """Load one env file into environ without overriding existing variables."""
    target_environ = os.environ if environ is None else environ
    return _env_file_loader().apply_file(path, target_environ)


def load_voicemode_env(environment: MutableMapping[str, str] | None = None):
    ...
    _env_file_loader().load(target_environ)
...
# TTS speed is parsed in the RuntimeContext-backed provider settings below.
...
# Provider settings are parsed by RuntimeContext; keep legacy globals as
# compatibility snapshots for existing imports.
_provider_settings = _build_provider_settings(os.environ)
TTS_SPEED = _provider_settings.tts_speed
OPENAI_API_KEY = _provider_settings.openai_api_key
ELEVENLABS_API_KEY = _provider_settings.elevenlabs_api_key
ELEVENLABS_TTS_MODEL = _provider_settings.elevenlabs_tts_model
ELEVENLABS_TTS_VOICE = _provider_settings.elevenlabs_tts_voice
ELEVENLABS_STT_MODEL = _provider_settings.elevenlabs_stt_model
ELEVENLABS_USE_REALTIME_STT = _provider_settings.elevenlabs_use_realtime_stt
TTS_BASE_URLS = list(_provider_settings.tts_base_urls)
STT_BASE_URLS = list(_provider_settings.stt_base_urls)
TTS_VOICES = list(_provider_settings.tts_voices)
TTS_MODELS = list(_provider_settings.tts_models)
...
def reload_configuration():
    ...
    # Update legacy provider globals from the RuntimeContext parser.
    global TTS_SPEED, OPENAI_API_KEY, ELEVENLABS_API_KEY, ELEVENLABS_TTS_MODEL
    global ELEVENLABS_TTS_VOICE, ELEVENLABS_STT_MODEL, ELEVENLABS_USE_REALTIME_STT
    global TTS_VOICES, TTS_MODELS, TTS_BASE_URLS, STT_BASE_URLS

    provider_settings = _build_provider_settings(os.environ)
    TTS_SPEED = provider_settings.tts_speed
    OPENAI_API_KEY = provider_settings.openai_api_key
    ELEVENLABS_API_KEY = provider_settings.elevenlabs_api_key
    ELEVENLABS_TTS_MODEL = provider_settings.elevenlabs_tts_model
    ELEVENLABS_TTS_VOICE = provider_settings.elevenlabs_tts_voice
    ELEVENLABS_STT_MODEL = provider_settings.elevenlabs_stt_model
    ELEVENLABS_USE_REALTIME_STT = provider_settings.elevenlabs_use_realtime_stt
    TTS_BASE_URLS = list(provider_settings.tts_base_urls)
    STT_BASE_URLS = list(provider_settings.stt_base_urls)
    TTS_VOICES = list(provider_settings.tts_voices)
    TTS_MODELS = list(provider_settings.tts_models)
...
# Trusted reverse proxies allowed to set X-Forwarded-For (comma-separated, default: empty)
