...
async def is_provider_available(provider_id: str, timeout: float = 2.0) -> bool:
    ...
    service_type = "tts" if provider_id in ["kokoro", "openai", "elevenlabs"] else "stt"
...
def get_provider_by_voice(voice: str) -> Optional[Dict[str, Any]]:
    ...
    from .config import ELEVENLABS_API_KEY, ELEVENLABS_TTS_VOICE
    ...
    if voice.startswith(('af_', 'am_', 'bf_', 'bm_')):
        # If ElevenLabs is configured, prefer it over Kokoro
    ...
    # OpenAI voices
    openai_voices = ["alloy", "nova", "echo", "fable", "onyx", "shimmer"]
    if voice in openai_voices:
        # If ElevenLabs is configured, prefer it
        if ELEVENLABS_API_KEY:
            return {
                "id": "elevenlabs",
                "name": "ElevenLabs TTS",
