...
def exchange_log_filename(target_date: Union[date, datetime]) -> str:
    """Return the persisted JSONL filename for an exchange log date."""
    if isinstance(target_date, datetime):
        target_date = target_date.date()

    return f"exchanges_{target_date.strftime('%Y-%m-%d')}.jsonl"
...
class ExchangeMetadata:
    ...
    voice_mode_version: Optional[str] = None
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ExchangeMetadata":
        """Create from dictionary while preserving unknown metadata fields."""
        known_fields = set(cls.__annotations__) - {"extra"}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            k: v for k, v in self.__dict__.items() if k != "extra" and v is not None
        }
        result.update({k: v for k, v in self.extra.items() if v is not None})
        return result
...
class Exchange:
    ...
    def from_jsonl(cls, line: str) -> "Exchange":
        ...
        return cls(
            ...
            type=cast(Literal["stt", "tts"], exchange_type),
            text=data["text"],
