...
from voice_mode.exchanges.models import (
    Exchange,
    ExchangeMetadata,
    exchange_log_filename,
)
...
class ConversationLogger:
    ...
    def __init__(self, base_dir: Optional[Path] = None):
        ...
        self.reader = ExchangeReader(logs_dir=self.base_dir)
        self.writer = ExchangeWriter(logs_dir=self.base_dir)
    ...
    def _initialize_conversation_id(self):
        ...
        if last_entry:
            try:
                ...
                time_diff = (
                    datetime.now().astimezone() - last_timestamp
                ).total_seconds()

                # Check if within conversation window and same project
                if (
                    time_diff < self.CONVERSATION_GAP_MINUTES * 60
                    and last_entry.project_path == self.current_project_path
                ):
    ...
    def _get_last_log_entry(self) -> Optional[Exchange]:
        """Get the last valid exchange from today's or yesterday's log file."""
        today = datetime.now().date()
        yesterday = today - timedelta(days=1)
        return self.reader.read_latest_from_dates([today, yesterday])
    ...
    def _get_log_file_path(self, date: datetime.date) -> Path:
        ...
        return self.base_dir / exchange_log_filename(date)
    ...
    def _check_conversation_continuity(self):
        ...
        if last_entry and last_entry.conversation_id == self.conversation_id:
            try:
                ...
                if (
                    time_diff >= self.CONVERSATION_GAP_MINUTES * 60
                    or last_entry.project_path != self.current_project_path
                ):
                    self.conversation_id = self._generate_conversation_id()
            except Exception:
